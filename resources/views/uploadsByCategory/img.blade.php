...
<style>
 ...
 @foreach ($fotos as $foto)
      <div class="card"
        ...
        <img src="{{asset('storage_ctm/uploads_img/' . $foto->file)}}" alt="{{$foto->url_foto}}"
          style="border-top-left-radius: 10px; border-top-right-radius: 10px; object-fit: cover; height: 200px; width: 100%;">
        <div class="card-body" style="padding: 15px;">
          <h5 class="card-title" style="color: #333; font-size: 1.5rem; margin-bottom: 10px;">{{$foto->user->username}}</h5>
          <p class="card-text" style="color: #666; font-size: 1rem; line-height: 1.5;">{{$foto->caption}}</p>    
        </div>
      </div>
  @endforeach
    </div>
