  ...
  <div class="sidebar"
    ...
    <center>
      <ul class="nav flex-column mb-auto">
        ...
        <li>
          <a href="/uploads/insert/web" class="nav-link"
            style="color: white; padding: 10px 15px; border-radius: 5px; margin-bottom: 80px; font-size: 20px;">
            Upload
          </a>
        </li>
  ...
  @foreach ($pages as $page)
      <div class="card"
        style="width: 18rem; border: 1px solid #ddd; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); transition: transform 0.3s, box-shadow 0.3s;">
            <iframe style="border-top-left-radius: 10px; border-top-right-radius: 10px; object-fit: cover; height: 200px; width: 100%;" src="{{asset('storage_ctm/uploads_web/' . $page->file)}}" allowfullscreen frameborder="1" width="100px"></iframe>
        <div class="card-body" style="padding: 15px;">
          <h5 class="card-title" style="color: #333; font-size: 1.5rem; margin-bottom: 10px;">{{$page->user->username}}</h5>
          <p class="card-text" style="color: #666; font-size: 1rem; line-height: 1.5;">{{$page->caption}}</p>    
        </div>
      </div>
  @endforeach
    </div>
